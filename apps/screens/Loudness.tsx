...
const Loudness = () => {
  ...
  useEffect(() => {
    const updateLoudnessData = () => {
      const newLoudnessRating = Math.floor(Math.random() * 81) + 20;
      setLoudnessRating(newLoudnessRating);
      setUpdateCounter((prevCounter) => prevCounter + 1);
    };

    const intervalId = setInterval(updateLoudnessData, 5000);
  ...
  useEffect(() => {
    ...
    setLoudnessHistory((prevHistory) => [...prevHistory.slice(-9), newArray[0]]);
    setUpdateCounter((prevCounter) => prevCounter + 1);
  }, [loudnessRating]);
  
  useEffect(() => {
    const lastTenRatings = loudnessHistory.slice(-10);
